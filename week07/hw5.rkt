  ...
  (cond [(var? e) 
        ...
        [(ifgreater? e) (let ([v1 (eval-under-env (ifgreater-e1 e) env)]
                          ...
                          (if (and (int? v1) (int? v2))
                              ...
                              (error "MUPL ifgreater applied to non-boolean")))]
        [(fun? e) (closure env e)]
        [(call? e) (let ([c (eval-under-env (call-funexp e) env)]
                     ...
                     (if (not (closure? c))
                         ...
                         (let* ([f (closure-fun c)]
                                [new-env (cons (cons (fun-formal f) p) (closure-env c))]
                                [n (fun-nameopt f)])
                           (if (not (equal? n #f))
                               (eval-under-env (fun-body f) (cons (cons n c) new-env))
        ...
        [(fst? e) (let ([v (eval-under-env (fst-e e) env)])
                    (if (apair? v)
                        (apair-e1 v)
                        (begin ((print v) error "MUPL fst applied to non-pair"))))]
...
(define (mlet* lstlst e2)
  (letrec ([f (lambda (l) (if (null? l)
                           e2
                           (mlet (car (car l)) (cdr (car l)) (f (cdr l)))))])
    (f lstlst)))
