...
class Point < GeometryValue
  ...
  def intersectLine line
    return self if real_close(y, line.m * x + line.b)
    NoPoints.new
  end
  def intersectVerticalLine vline
  ...
  def intersectWithSegmentAsLineResult seg
    return self if inbetween(x, seg.x1, seg.x2) && inbetween(y, seg.y1, seg.y2)
    NoPoints.new
  end
...
class LineSegment < GeometryValue
  ...
  def preprocess_prog
    ...
    return LineSegment.new(x2, y2, x1, y1) if improper?
    self
  end
  def shift(dx, dy)
